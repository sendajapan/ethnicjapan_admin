...
@section('content')
    <section class="content-main">
        <div class="content-header">
            <div>
                <h4 class="content-title card-title">Update Port Information</h4>
            </div>
        </div>
        <div class="row">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <h5>Please enter bank information</h5>
                            <form action="{{ route('admin.ports.update', $data->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="mb-4">
                                <label for="port_name" class="form-label">Port Name</label>
                                <input type="text" placeholder="" class="form-control" id="port_name" name="port_name"  value="{{ $data->port_name }}" required>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select" id="country_name" name="country_name" required >
                                    ...
                                    @foreach(Countries::orderBy('country_name')->get() as $p)
                                        <option value="{{ $p->country_name }}"  {{ $p->country_name == $data->country_name ? 'selected' : '' }} >{{ $p->country_name }}</option>
                                    @endforeach
                                </select>
                            </div>
