...
class SaleReportController extends Controller
{
    ...
    public function index()
    {
        ...
        $purchases = SaleItem::leftJoin('sales', 'sale_items.sale_id', 'sales.id')
        ...
        $customers = Customer::all()->sortBy('provider_name');
        ...
        foreach($purchases as $p){
            $year = date('Y', strtotime($p->sale_date));
            $month = date('m', strtotime($p->sale_date));
        ...
        for($year = date("Y"); $year>=$year_starting;  $year--){
            foreach ($customers as $p) {
                $purchase_info_all[$year]['by_customer'][$p->customer_name] = 0;
            }
        }
        foreach($purchases as $p){
            $year = date('Y', strtotime($p->sale_date));
            $purchase_info_all[$year]['by_customer'][$p->customer_name]  += $p->item_line_price;
        }
        ...
        return view('admin.sale_report.index', compact('data'));
    }
